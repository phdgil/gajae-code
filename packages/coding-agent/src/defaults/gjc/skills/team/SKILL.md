...
5. If current correctness depends on official docs, version-aware framework guidance, best practices, or external dependency behavior, auto-delegate an evidence lane before or alongside worker launch instead of relying on repo-local recall alone.
...
- `GJC_TMUX_COMMAND` / `GJC_TEAM_TMUX_COMMAND`
  ...
  - Multiplexer support boundary: GJC-managed sessions and the team leader are detected via tmux user options (`@gjc-profile`, written with `set-option` and read back with `show-options` / `list-sessions -F`). When a provider drops those user options, GJC falls back to its own ownership sidecar for sessions it created itself. On Windows, GJC now prefers `psmux` automatically when available; foreign psmux/tmux sessions without a GJC tag or sidecar still remain unmanaged by design.
